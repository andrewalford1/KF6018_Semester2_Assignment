...
class StreetLamp extends ENGINE.OBJECTS.ClassicObject
{
    ...
    constructor(position)
    {
        ...
        //let pointLight = new THREE.PointLight( 0xFFFBEF, 2, 100, 1);
        //pointLight.distance = 100;
        //pointLight.position.set(-20, 57, -400);
        ...
        //this.addObjectToGroup( pointLightHelper );
        //For loop to create the spot lights for the street lamps
        for(let s = 0; s < streetLightNumber; s++){
                //Spot light for the street lamps
                streetLight.push(new THREE.PointLight( 0xFFFBEF, 2.5, 130, 1));
                streetLight[s].castShadow = true;
                //Adjusting the shadow casted by the spot light
                streetLight[s].shadow.mapSize.width = 1024;
                streetLight[s].shadow.mapSize.height = 1024;
                streetLight[s].shadow.camera.near = 20;
                //streetLight[s].shadow.camera.far = 4000;
                //streetLight[s].shadow.camera.fov = 30;
                this.addObjectToGroup( streetLight[s] );
        }
